...
class PriceCache:
    ...
    def _load_cache(self) -> None:
        ...
        if self._price_cache_file.exists():
            try:
                with open(self._price_cache_file) as f:
                    data = json.load(f)
        ...
        if self._product_cache_file.exists():
            try:
                with open(self._product_cache_file) as f:
                    data = json.load(f)
                    now = time.time()
                    for pid, entry in data.items():
        ...
        if self._search_cache_file.exists():
            try:
                with open(self._search_cache_file) as f:
                    data = json.load(f)
                    now = time.time()
    ...
    def get_price(self, product_id: str) -> CachedPrice | None:
        """
    ...
    def get_search(self, query: str, language: str = None) -> list[dict] | None:
    ...
    def clear_expired(self) -> dict[str, int]:
        ...
        # Clean price cache
        expired_prices = [pid for pid, cp in self._price_cache.items() if cp.is_expired()]
        ...
        # Clean product cache
        expired_products = [
            pid for pid, entry in self._product_cache.items() if now > entry.get("expires_at", 0)
        ]
        ...
        # Clean search cache
        expired_searches = [
            key for key, entry in self._search_cache.items() if now > entry.get("expires_at", 0)
        ]
...
_cache_instance: PriceCache | None = None
