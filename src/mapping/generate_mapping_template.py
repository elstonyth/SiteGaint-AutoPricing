...
def create_mapping_template(sitegiant_df: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Column mapping: name='{col_name}', isku='{col_isku}', sku='{col_sku}', price='{col_price}'")
...
def get_config_paths() -> tuple[Path, Path]:
    ...
    if config_file.exists():
        ...
        output_path = Path(paths.get("mapping_template_path", "data/mapping/sitegiant_pokedata_mapping_template.xlsx"))
    else:
...
def main() -> int:
    ...
    try:
        ...
        print(f"  2. For each product, find it on https://www.pokedata.io/products")
        print(f"  3. Fill in these columns:")
        print(f"     - pokedata_id:   Enter the Pokedata product ID")
        print(f"     - pokedata_name: Copy the product name (for reference)")
        print(f"     - pokedata_url:  Paste the Pokedata product URL (optional)")
        print(f"  4. Set auto_update to 'N' for any products you want to skip")
        ...
        if args.input_file is None:
            ...
            print(f"        Or provide a path: python -m src.mapping.generate_mapping_template path/to/export.xlsx")
        return 1
