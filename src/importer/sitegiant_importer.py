...
class SiteGiantImporter:
    ...
    def import_products(self, file_path: Path) -> pd.DataFrame:
        ...
        if missing:
            raise ValueError(f"Missing required columns in SiteGiant export: {missing}. "
...
def load_sitegiant_export(file_path: Path, config: Optional[AppConfig] = None) -> pd.DataFrame:
    """
