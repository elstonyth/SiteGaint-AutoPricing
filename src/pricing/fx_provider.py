...
def fetch_google_fx_rate(config: AppConfig) -> tuple[float | None, str]:
    """
...
def get_fx_rate(config: AppConfig, manual_override: float | None = None) -> tuple[float, str]:
    """
    ...
    if mode.lower() == "google":
        ...
        if rate is not None:
            ...
            logger.warning(
                f"Google FX fetch failed ({source}), falling back to default rate: {default_rate}"
            )
