...
class PricingService:
    ...
    def process(
        ...
        pokedata_prices, demo_mode, demo_warning = self._fetch_prices(
            df, config, fx_rate
        )
    ...
    def _resolve_fx_rate(self, override: Optional[float]) -> Tuple[float, str]:
        ...
        if api_key:
            try:
                ...
                if unique_ids:
                    ...
                    valid_prices = sum(
                        1 for p in pokedata_prices.values()
    ...
    def _get_unique_pokedata_ids(self, df: pd.DataFrame) -> List[str]:
        ...
        return [
            ...
            if pid and str(pid).strip() and str(pid).lower() != "nan"
        ]
        ...
        for _, row in df[df["is_mapped"]].iterrows():
            ...
            if pd.notna(current_price) and float(current_price) > 0:
                ...
                demo_prices[pid] = type(
                    "DemoPrice", (),
                    {"primary_price_usd": simulated_usd, "source": "DEMO"}
                )()
