...
from src.storage.settings_store import get_api_key, get_settings
...
def has_valid_api_key(override: Optional[str] = None) -> bool:
...
def validate_mapping_file(df: pd.DataFrame, filename: str) -> Dict[str, Any]:
    ...
    if "pokedata_language" in df_columns_lower:
        ...
        invalid_langs = df[~df[lang_col].astype(str).str.upper().isin(valid_langs)][lang_col].dropna().unique()
    ...
    if "auto_update" in df_columns_lower:
        ...
        invalid_auto = df[~df[auto_col].astype(str).str.upper().isin(valid_auto)][auto_col].dropna().unique()
...
def get_friendly_upload_error(error: Exception, filename: str) -> str:
    ...
    if "no such file" in error_str or "file not found" in error_str:
        ...
        return f"Unsupported file format. Please upload .xlsx, .xls, or .csv files only."
...
def format_results_for_display(df: pd.DataFrame) -> List[Dict[str, Any]]:
    ...
    display_cols = [
        "name", "sku", "price", "new_price_myr", "abs_change", "pct_change",
        "status", "status_reason", "auto_update", "include_in_update"
    ]
