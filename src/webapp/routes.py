...
from src.mapping.mapping_manager import load_mapping
...
from src.services.pricing_service import ProcessingConfig as ServiceProcessingConfig
from src.storage.session_store import (
    cleanup_sessions,
    create_session,
    delete_session,
)
from src.storage.session_store import get_session as get_stored_session
...
from src.storage.settings_store import has_api_key as stored_has_api_key
...
async def process(
    ...
    api_key_override: str | None = Form(None),
    ...
    try:
        ...
        if filename_lower.endswith(".csv"):
            ...
            raise ValueError(
                f"Unsupported file format: {sitegiant_file.filename}. Please upload .xlsx, .xls, or .csv files."
            )
        ...
        service_config = ServiceProcessingConfig(
            ...
            margin_divisor=margin_divisor,
            api_key_override=(
                api_key_override if api_key_override and api_key_override.strip() else None
            ),
        ...
        products_updated = (
            ...
            if "include_in_update" in results_df.columns
            else 0
        )
...
async def export(
            ...
            if mask.any():
                ...
                if pd.notna(old_price) and abs(float(old_price) - new_price) > 0.01:
                    ...
                    if pd.notna(current_price) and current_price > 0:
                        ...
                        results_df.loc[mask, "pct_change"] = (
    ...
    if demo_mode:
        ...
        results_list = format_results_for_display(results_df)
        fx_rate_used = (
            results_df["fx_rate_used"].iloc[0]
            if "fx_rate_used" in results_df.columns
            else config.get("fx_rate", 0)
        )
        fx_source_used = (
            results_df["fx_source"].iloc[0] if "fx_source" in results_df.columns else "unknown"
        )
        demo_warning = (
            results_df["demo_mode_warning"].iloc[0]
            if "demo_mode_warning" in results_df.columns
            else ""
        )
    ...
    try:
        ...
        export_records = []
        exported_df = results_df[results_df["include_in_update"]]
...
async def save_api_key(
    ...
    if api_key and api_key.strip():
        ...
        return RedirectResponse(
            url="/settings/mapping?success=API+key+saved+successfully.", status_code=303
        )
    else:
        return RedirectResponse(
            url="/settings/mapping?error=API+key+cannot+be+empty.", status_code=303
        )
...
async def clear_api_key_route():
    ...
    clear_api_key()
    return RedirectResponse(url="/settings/mapping?success=API+key+cleared.", status_code=303)
...
async def save_config(
    ...
    return RedirectResponse(
        url="/settings/mapping?success=Configuration+saved+successfully.", status_code=303
    )
    ...
    try:
        ...
        if auto_lookup and api_key:
            ...
            for idx in df.index:
                ...
                language = (
                    df.at[idx, "pokedata_language"]
                    if "pokedata_language" in df.columns
                    else "ENGLISH"
                )
        ...
        if auto_lookup and (
            lookup_results["found"] or lookup_results["not_found"] or lookup_results["errors"]
        ):
            mapping_info = get_mapping_info(mapping_path)
            settings = get_settings()
            api_key_set = stored_has_api_key()
...
async def confirm_mapping_upload(
    ...
    try:
        ...
        if strategy == "ignore":
            ...
            logger.info("Applied 'ignore' strategy: kept first entry for duplicates")
        else:  # "merge" - keep last
            df = df.drop_duplicates(subset=["_sku_normalized"], keep="last")
            logger.info("Applied 'merge' strategy: kept last entry for duplicates")
        ...
        logger.info(
            f"Saved mapping file with {len(df)} rows (removed {duplicates_removed} duplicates using '{strategy}' strategy)"
        )

        success_msg = f"Mapping file saved successfully. {len(df)} rows saved"
        if duplicates_removed > 0:
            success_msg += (
                f" ({duplicates_removed} duplicate(s) handled using '{strategy}' strategy)"
            )
        ...
        return RedirectResponse(
            url=f"/settings/mapping?success={success_msg.replace(' ', '+')}", status_code=303
        )
...
async def refresh_mapping(request: Request):
    ...
    try:
        ...
        for idx in df.index:
            ...
            pokedata_id = df.at[idx, "pokedata_id"] if "pokedata_id" in df.columns else None
            language = (
                df.at[idx, "pokedata_language"] if "pokedata_language" in df.columns else "ENGLISH"
            )
...
async def _lookup_pokedata_id(
    config, pokedata_url: str, pokedata_language: str = "ENGLISH"
) -> dict:
...
async def generate_mapping_template(
    ...
    """
    from src.mapping.generate_mapping_template import (
        ...
        try:
            ...
            if search_mode == "name":
                ...
                if results:
                    ...
                    error = (
                        f"No products found for '{search_query}'. The API returned empty results."
                    )
            else:
