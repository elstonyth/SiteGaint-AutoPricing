...
class ProcessingConfig(BaseModel):
    ...
    fx_rate: float | None = Field(
        None, ge=1.0, le=10.0, description="USD to MYR exchange rate override (1.0-10.0)"
    )
    soft_threshold: float = Field(
        20.0, ge=0.0, le=100.0, description="Soft threshold percentage - shows warning"
    )
    hard_threshold: float = Field(
        50.0, ge=0.0, le=100.0, description="Hard threshold percentage - blocks export"
    )
    margin_divisor: float = Field(
        0.8, ge=0.1, le=1.0, description="Margin divisor for price calculation (0.1-1.0)"
    )
    api_key_override: str | None = Field(
        None, description="Override API key for this session only"
    )
    force_refresh: bool = Field(False, description="Force refresh prices from API (bypass cache)")
...
class ExportRequest(BaseModel):
    ...
    session_id: str = Field(..., min_length=1, description="Session ID for the processing results")
    selected_skus: list[str] = Field(
        default_factory=list, description="List of SKUs to include in export"
    )
    edited_prices: dict[str, float] = Field(
        default_factory=dict, description="Optional price overrides: {sku: new_price}"
    )
...
class MappingEntryRequest(BaseModel):
    ...
    pokedata_id: str | None = Field(None, description="Pokedata product ID")
    pokedata_url: str | None = Field(None, description="Pokedata product URL")
    pokedata_name: str | None = Field(None, description="Product name on Pokedata")
