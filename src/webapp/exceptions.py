...
class FileValidationError(ValidationError):
    ...
    def __init__(
        self, message: str, filename: str | None = None, missing_columns: list | None = None
    ):
        details = {}
        if filename:
...
class SessionNotFoundError(SessionError):
    ...
    def __init__(self, session_id: str):
        super().__init__(
            f"Session not found or expired: {session_id[:8]}...", details={"session_id": session_id}
        )
...
class ExportBlockedError(AppException):
    ...
    def __init__(self, reason: str):
        super().__init__(f"Export blocked: {reason}", details={"reason": reason})
