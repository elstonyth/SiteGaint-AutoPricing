...
from src.storage.price_cache import PriceCache, get_cache, CachedPrice
...
class PokedataClient:
            ...
            try:
                ...
                if response.status_code == 500:
                    ...
                    try:
                        ...
                        error_msg = response.text[:200] if response.text else "Internal Server Error"
                ...
                if response.status_code == 429:
                    ...
                    if attempt < max_retries - 1:
                        logger.info(f"Retrying request after rate limit (attempt {attempt + 2}/{max_retries})")
                        continue
                    else:
    ...
    def get_product(self, product_id: str) -> Optional[PokedataProduct]:
        """
    ...
    def get_prices_batch(
        ...
        if self._use_cache and not force_refresh:
            ...
            if cache_hits > 0:
                logger.info(
                    f"Cache: {cache_hits}/{total} hits, "
                    f"{len(ids_to_fetch)} API calls needed"
                )
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_id = {
                executor.submit(fetch_single, pid): pid 
    ...
    def search_products(
        ...
        try:
            ...
            logger.info(f"Search API response type: {type(data)}, keys: {data.keys() if isinstance(data, dict) else 'N/A'}")
