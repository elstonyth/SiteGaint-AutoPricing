...
class TestPokedataURLParsing:
    ...
    def test_parse_single_url_english(self, client):
        ...
        response = client.post(
            "/api/parse-pokedata-url", data={"url": "https://www.pokedata.io/product/66"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_parse_single_url_japanese(self, client):
        ...
        response = client.post(
            "/api/parse-pokedata-url", data={"url": "https://www.pokedata.io/jp/product/89"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_valid"] is True
    ...
    def test_parse_invalid_url(self, client):
        ...
        response = client.post("/api/parse-pokedata-url", data={"url": "https://google.com"})
        assert response.status_code == 200
        data = response.json()
...
https://invalid-url.com"""
        ...
        response = client.post("/api/parse-pokedata-urls", data={"urls": urls})
        assert response.status_code == 200
        data = response.json()
        assert data["total"] == 3
        assert data["valid"] == 2
    ...
    def test_validate_id_numeric(self, client):
        ...
        response = client.post("/api/validate-pokedata-id", data={"pokedata_id": "66"})
        assert response.status_code == 200
        data = response.json()
        assert data["is_valid"] is True
    ...
    def test_validate_id_from_url(self, client):
        ...
        response = client.post(
            "/api/validate-pokedata-id", data={"pokedata_id": "https://www.pokedata.io/product/123"}
        )
        assert response.status_code == 200
        data = response.json()
        assert data["is_valid"] is True
